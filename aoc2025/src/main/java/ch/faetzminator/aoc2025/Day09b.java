...
public class Day09b {
    ...
    public long getSolution() {
        ...
        for (int i = 0; i < points.size(); i++) {
            for (int j = i + 1; j < points.size(); j++) {
                if (isValid(points.get(i), points.get(j))) {
    ...
    private boolean isValid(final Point a, final Point b) {
        final int xa = Math.min(a.getX(), b.getX());
        final int ya = Math.min(a.getY(), b.getY());
        final int xb = Math.max(a.getX(), b.getX());
        final int yb = Math.max(a.getY(), b.getY());

        Point last = points.get(points.size() - 1);
        for (final Point point : points) {
            final int x1 = Math.min(point.getX(), last.getX());
            final int y1 = Math.min(point.getY(), last.getY());
            final int x2 = Math.max(point.getX(), last.getX());
            final int y2 = Math.max(point.getY(), last.getY());
